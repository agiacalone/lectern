...
- **`reg-term-create` — `{{archive}}` was never substituted in generated class notes.** `_class_note()` built its `_expand()` mapping without `archive` or `course-dir`, so the class-note template's Sources block kept a literal `{{archive}}` placeholder in every note the command produced (still visible in the Su26 378-01 note). Where the link had been hand-substituted after the fact, it addressed the bundle *directory* — and Obsidian resolves wikilinks to notes, not folders, so it was dead whether or not the bundle had been built. Both keys are passed through now, letting the template target `classes/<dir>/archives/<term>-<sec>/README`, which is the note a built bundle actually has; unbuilt bundles read as pending rather than broken. The template itself lives in the vault (`templates/class-note.md`) and is not versioned here — the same pass repointed its semester rollup from `classes/<term>` to `classes/semesters/<term>`, a path that has never existed.
