...
def test_read_eval_counts_and_skips(tmp_path):
    ...
    p = tmp_path / "1.csv"
    _eval(p, cols, [
        (101, "040100204", 2.0, [False, True, False, False]),   # correct
        (102, "040100213", 0.0, [False, False, True, False]),   # wrong (c)
...
def test_read_eval_flags_anomaly(tmp_path):
    ...
    p = tmp_path / "1.csv"
    _eval(p, cols, [(101, "040100204", 23.2, [True, False])])   # 23.2 on a 2-pt Q
    n, pairs, scores, anom, skipped = read_eval(p, 2)
...
def test_exam_summary(tmp_path):
    ...
    p.write_text(
        ...
        "Gordon,James,040100204,A,45.0,Graded\n"     # 90% → A
        "Doe,Jane,040100213,B,35.0,Graded\n"      # 70% → C
        "Cain,Cassandra,040100214,A,15.0,Graded\n"     # 30% → F
...
def stats_tree(tmp_path, note_file):
    ...
    _eval(ev / "groupA" / "1_cia.csv",
          ...
          [(1, "040100204", 2.0, [False, True, False, False]),
           (2, "040100213", 2.0, [False, True, False, False]),
           (3, "040100214", 0.0, [False, False, True, False])])
    ...
    _eval(ev / "groupA" / "2_fib.csv",
          ...
          [(1, "040100204", 0.0, [True, True, False]),
           (2, "040100213", 0.0, [True, False, False])])
    scores = tmp_path / "scores.csv"
    scores.write_text("last,first,sid,version,score,status\n"
                      "Gordon,James,040100204,A,2.0,Graded\n", encoding="utf-8")
