...
def _rows():
    return [
        {"student_id": "040100204", "display_name": "James Gordon", "enrollment_status": "enrolled",
...
def _exam_rows():
    return [
        {"student_id": "040100204", "display_name": "James Gordon", "raw_scores": json.dumps({"exam1": 40.0})},
        {"student_id": "040100215", "display_name": "Sena M", "raw_scores": json.dumps({"exam1": 0.0})},
    ]
...
def test_render_assignment_page_exam_with_grid(tmp_path, schema_378):
    ...
    assert "James Gordon" in md and "40" in md                          # score roster
...
def test_render_assignment_page_lab_no_grid(tmp_path, schema_378):
    ...
    rows = [{"student_id": "040100204", "display_name": "James Gordon",
             "raw_scores": json.dumps({"lab1": 55.0})}]
    md = render_assignment_page(entry, schema_378, rows)
    assert "# Lab 1" in md and "James Gordon" in md and "55" in md
...
def test_reconcile_assignment_flags_mismatch(tmp_path, schema_378):
    ...
    assert any("040100204" in f and "41" in f and "40" in f for f in result["mismatches"])
...
def test_reconcile_flags_roster_mismatches(tmp_path, schema_378):
    ...
    # 040100204 is in _exam_rows() (recorded), 040100215 is also in _exam_rows()
    # Matrix has 040100204 (in both) + 099999999 (grid-only, not in rows)
    # 040100215 is in rows but NOT in matrix → "recorded but no grid row"
    ...
    # 040100215 scored 0.0 in _exam_rows but is absent from the matrix
    assert "040100215" in sid_reconciling and "recorded but no grid row" in sid_reconciling
...
def test_render_assignment_page_stats(tmp_path, schema_378):
    ...
    # James Gordon: 40/50 = 80% → B band; Sena M: 0/50 = 0% → F band
    assert "B:1" in md
    assert "F:1" in md
...
def test_render_assignment_page_lab_per_criterion(tmp_path, schema_378):
    ...
    scores.write_text(
        ...
        "Gordon,James,040100204,,55,Graded,10\n",
        encoding="utf-8",
    )
...
def test_render_assignment_page_lab_no_extra_columns(tmp_path, schema_378):
    ...
    scores.write_text(
        ...
        "Gordon,James,040100204,,55,Graded\n",
        encoding="utf-8",
    )
    entry = RegistryEntry(short_name="lab1", scores_path=scores, kind="lab")
    rows = [{"student_id": "040100204", "display_name": "James Gordon",
...
def test_render_assignment_page_two_forms(tmp_path, schema_378):
    ...
    rows = _exam_rows()  # 040100204 → 40.0, 040100215 → 0.0
    md = render_assignment_page(entry, schema_378, rows)
...
def test_reconcile_mismatch_vs_reconciling_routing(tmp_path, schema_378):
    ...
    # 040100204: grid says 41, recorded says 40 → value mismatch
    # 040100215: recorded 0.0 but absent from matrix → reconciling item (no-show)
    matrix = tmp_path / "item_scores_A.csv"
    matrix.write_text(
        "student_id,Q1,Q2,total\n040100204,20,21,41\n",
        encoding="utf-8",
    ...
    entry = RegistryEntry(short_name="exam1", scores_path=tmp_path / "x.csv",
                          breakdown=(matrix,), kind="exam")
    result = reconcile_assignment(entry, _exam_rows())
    # 040100204 grid 41 ≠ recorded 40 → mismatches
    assert any("040100204" in m and "41" in m for m in result["mismatches"])
    # 040100215 in recorded but not in grid → reconciling
    assert any("040100215" in r and "recorded but no grid row" in r for r in result["reconciling"])
    # Sanity: buckets don't bleed
    assert not any("040100215" in m for m in result["mismatches"])
    assert not any("040100204" in r for r in result["reconciling"])
...
def test_render_page_only_reconciling_items_uses_note_not_danger(tmp_path, schema_378):
    ...
    # 040100204: grid total matches recorded (40 = 40) — balanced
    # 040100215: recorded 0.0 but absent from matrix — reconciling item only
    matrix = tmp_path / "item_scores_A.csv"
    matrix.write_text(
        "student_id,Q1,Q2,total\n040100204,20,20,40\n",
        encoding="utf-8",
