...
def _rows():
    return [
        {"student_id": "034935496", "display_name": "Ryan Bell", "enrollment_status": "enrolled",
...
def _exam_rows():
    return [
        {"student_id": "034935496", "display_name": "Ryan Bell", "raw_scores": json.dumps({"exam1": 40.0})},
        {"student_id": "033247550", "display_name": "Sena M", "raw_scores": json.dumps({"exam1": 0.0})},
    ]
...
def test_render_assignment_page_exam_with_grid(tmp_path, schema_378):
    ...
    assert "Ryan Bell" in md and "40" in md                          # score roster
...
def test_render_assignment_page_lab_no_grid(tmp_path, schema_378):
    ...
    rows = [{"student_id": "034935496", "display_name": "Ryan Bell",
             "raw_scores": json.dumps({"lab1": 55.0})}]
    md = render_assignment_page(entry, schema_378, rows)
    assert "# Lab 1" in md and "Ryan Bell" in md and "55" in md
...
def test_reconcile_assignment_flags_mismatch(tmp_path, schema_378):
    ...
    assert any("034935496" in f and "41" in f and "40" in f for f in result["mismatches"])
...
def test_reconcile_flags_roster_mismatches(tmp_path, schema_378):
    ...
    # 034935496 is in _exam_rows() (recorded), 033247550 is also in _exam_rows()
    # Matrix has 034935496 (in both) + 099999999 (grid-only, not in rows)
    # 033247550 is in rows but NOT in matrix → "recorded but no grid row"
    ...
    # 033247550 scored 0.0 in _exam_rows but is absent from the matrix
    assert "033247550" in sid_reconciling and "recorded but no grid row" in sid_reconciling
...
def test_render_assignment_page_stats(tmp_path, schema_378):
    ...
    # Ryan Bell: 40/50 = 80% → B band; Sena M: 0/50 = 0% → F band
    assert "B:1" in md
    assert "F:1" in md
...
def test_render_assignment_page_lab_per_criterion(tmp_path, schema_378):
    ...
    scores.write_text(
        ...
        "Bell,Ryan,034935496,,55,Graded,10\n",
        encoding="utf-8",
    )
...
def test_render_assignment_page_two_forms(tmp_path, schema_378):
    ...
    rows = _exam_rows()  # 034935496 → 40.0, 033247550 → 0.0
    md = render_assignment_page(entry, schema_378, rows)
...
def test_reconcile_mismatch_vs_reconciling_routing(tmp_path, schema_378):
    ...
    # 034935496: grid says 41, recorded says 40 → value mismatch
    # 033247550: recorded 0.0 but absent from matrix → reconciling item (no-show)
    matrix = tmp_path / "item_scores_A.csv"
    matrix.write_text(
        "student_id,Q1,Q2,total\n034935496,20,21,41\n",
        encoding="utf-8",
    ...
    entry = RegistryEntry(short_name="exam1", scores_path=tmp_path / "x.csv",
                          breakdown=(matrix,), kind="exam")
    result = reconcile_assignment(entry, _exam_rows())
    # 034935496 grid 41 ≠ recorded 40 → mismatches
    assert any("034935496" in m and "41" in m for m in result["mismatches"])
    # 033247550 in recorded but not in grid → reconciling
    assert any("033247550" in r and "recorded but no grid row" in r for r in result["reconciling"])
    # Sanity: buckets don't bleed
    assert not any("033247550" in m for m in result["mismatches"])
    assert not any("034935496" in r for r in result["reconciling"])
...
def test_render_page_only_reconciling_items_uses_note_not_danger(tmp_path, schema_378):
    ...
    # 034935496: grid total matches recorded (40 = 40) — balanced
    # 033247550: recorded 0.0 but absent from matrix — reconciling item only
    matrix = tmp_path / "item_scores_A.csv"
    matrix.write_text(
        "student_id,Q1,Q2,total\n034935496,20,20,40\n",
        encoding="utf-8",
