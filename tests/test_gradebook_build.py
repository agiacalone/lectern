...
def test_build_gradebook_withdrawn_yields_W(tmp_path, schema_378):
    ...
    sc = tmp_path / "exam1_scores.csv"
    _write_scores(sc, [
        "Brown,Stephanie,895444082,A,40.0,Graded",       # enrolled → B
        "Fox,Lucius,406974877,A,45.0,Graded",    # withdrawn but 45/50=90% → still W
    ])
    reg = tmp_path / "components.yaml"
    reg.write_text("components:\n  - short_name: exam1\n    scores: exam1_scores.csv\n",
                   encoding="utf-8")
    roster = _roster(tmp_path, [
        "895444082,Stephanie Brown,enrolled",
        "406974877,Lucius Fox,withdrawn",
    ])
    out = tmp_path / "out"; out.mkdir()
    rows = build_gradebook(reg, roster, schema, out)
    by = {r["student_id"]: r for r in rows}
    assert by["406974877"]["letter_grade"] == "W"          # withdrawal trumps the 90%
    assert "withdrew" in by["406974877"]["flags"]
    assert by["406974877"]["enrollment_status"] == "withdrawn"
    assert by["895444082"]["letter_grade"] == "B"          # enrolled student unaffected
